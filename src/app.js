...
`;
          ...
          <div class="card">
            <div class="card-head">
              <div class="card-head-copy">
                ...
                <div class="card-note" id="univ-top-note">${buildUnivTopNoteHtml("결과 비율")}</div>
              </div>
              <div class="control-stack">
                <div class="control-group">
                  <div class="control-group-label">표시 방식</div>
                  <div class="segmented-control" data-univ-top-mode-group="global" aria-label="대학 TOP 20 차트 표시 방식">
                ...
                <div class="control-group">
                  ...
                  <div class="segmented-control" data-univ-top-sort-group="global" aria-label="대학 TOP 20 정렬 기준">
    ...
    function renderYearOverviewChart(allRecords, years, containerId) {
      ...
      const traces = [
        { x: yearLabels, y: passArr, name: "합격", type: "bar", marker: { color: "#059669" } },
        { x: yearLabels, y: waitArr, name: "충원합격", type: "bar", marker: { color: "#d97706" } },
        { x: yearLabels, y: failArr, name: "불합격", type: "bar", marker: { color: "#dc2626" } },
        { x: yearLabels, y: rateArr, name: "합격률(%)", type: "scatter", mode: "lines+markers+text",
          text: rateArr.map(v => v.toFixed(1) + "%"), textposition: "top center",
          yaxis: "y2", line: { color: "#2563eb", width: 2.5 }, marker: { size: 7, color: "#2563eb" } },
      ];
      const layout = {
        barmode: "stack", font: { family: "Inter, sans-serif", size: 12 },
        xaxis: { title: "학년도", type: "category" },
        yaxis: { title: "건수", side: "left" },
        yaxis2: { title: "합격률(%)", overlaying: "y", side: "right", range: [0, 100], showgrid: false },
        legend: { orientation: "h", y: -0.2 }, margin: { l: 50, r: 50, t: 20, b: 60 },
      };
      Plotly.newPlot(el, traces, layout, { displayModeBar: false, responsive: true });
    }

    function renderYearGradeBoxPlot(allRecords, years, containerId) {
      const el = document.getElementById(containerId);
      if (!el) return;
      const traces = [];
      ...
      const layout = {
        boxmode: "group", font: { family: "Inter, sans-serif", size: 12 },
        xaxis: { title: "학년도", type: "category" },
        yaxis: { title: "전교과등급", autorange: "reversed" },
        legend: { orientation: "h", y: -0.2 }, margin: { l: 50, r: 30, t: 20, b: 60 },
      };
      Plotly.newPlot(el, traces, layout, { displayModeBar: false, responsive: true });
    }
      ...
      const bodyRows = top.map(([univ, data]) => {
        const cells = years.map(y => {
          ...
          const rate = d.total ? (d.pass / d.total * 100).toFixed(1) + "%" : "-";
          const passBandMeta = buildUnivPassBandMeta(d.passGrades || []);
          const rateCell = d.total
            ? renderDetailTooltipCell(rate, [
    ...
    function registerAuxRenderers(records, allRecords, metaYears) {
      ...
      if (document.getElementById("univ-top-chart")) {
        renderState.auxRendererMap["univ-top-chart"] = () => renderUnivTop(records);
	      }
	      const yrs = metaYears || [];
    ...
    function syncUnivTopModeButtons() {
      const group = document.querySelector("[data-univ-top-mode-group]");
      if (!group) return;
    ...
    function updateUnivTopNote() {
      const noteEl = document.getElementById("univ-top-note");
      if (!noteEl) return;
      const modeText = getUnivTopMode() === "ratio" ? "결과 비율" : "절대 건수";
      noteEl.innerHTML = buildUnivTopNoteHtml(modeText);
    }

    function bindUnivTopModeToggle(records) {
      const group = document.querySelector("[data-univ-top-mode-group]");
      if (!group) return;
    ...
    function syncUnivTopSortButtons() {
      const group = document.querySelector("[data-univ-top-sort-group]");
      if (!group) return;
    ...
    function bindUnivTopSortToggle(records) {
      const group = document.querySelector("[data-univ-top-sort-group]");
      if (!group) return;
    ...
    function getDetailDeptOptions(records = []) {
      const deptSet = new Set();
      records.forEach((record) => {
        deptSet.add(record.dept || "미상");
      });
      return Array.from(deptSet).sort((a, b) => a.localeCompare(b, "ko", { numeric: true }));
    }
    ...
    function getDetailRowsForView(view) {
      ...
      if (view.type === "univ") {
        ...
        detailRows = sortRowsByGrade(dataState.reportRecords.filter((r) => (r.region || "미상") === view.value));
      } else if (view.type === "gradeBand") {
        detailRows = sortRowsByGrade(dataState.reportRecords.filter((r) => {
        ...
        detailRows = dataState.reportRecords
